<?php require_once 'connect.php'; 
	...
	if (isset($_POST['register'])) {
		...
		if (count($input_error)==0) {
			$check_email= mysqli_query($con,"SELECT * FROM `admin_login` WHERE `email`='$email';");

			if (mysqli_num_rows($check_email)==0) {
				$check_username= mysqli_query($con,"SELECT * FROM `admin_login` WHERE `username`='$username';");
				if (mysqli_num_rows($check_username)==0) {
					if (strlen($username)>7) {
						if (strlen($password)>7) {
								$password = sha1(md5($password));
								$query = "INSERT INTO `admin_login`(`name`, `email`, `username`, `password`, `photo`, `status`) 
...
<html lang="en">
  ...
  <body>
    <div class="container"><br>
         <br> <h1 class="text-center">Register Account</h1><hr><br>
          <div class="d-flex justify-content-center">
          	<?php 
          		if (isset($_GET['insert'])) {
          			if($_GET['insert']=='sucess'){ echo '<div role="alert" aria-live="assertive" 
                        aria-atomic="true" align="center" class="toast alert alert-success fade hide" 
                        data-delay="2000">Your Data Inserted!</div>';}
          		}
          	;?>
          </div>
          <div class="row animate__animated animate__pulse">
            <div class="col-md-8 offset-md-2">
              ...
              <br><br><p>If you already have an account, you can <a href="loginn.php">Login</a> your account!</p>
    </div>
    <footer>
    	<p>Copyright &copy; 2020 to <?php echo date('Y') ?></p>
    </footer>
    <!-- Optional JavaScript -->
